...
export default async (req: CustomRequest<IAddProduct>, res: Response) => {
    try {
        ...
        if (whereToSell && whereToSell.length > 0) {
            productData = {...productData, whereToSell: whereToSell}
        }

        if (marketingAngel && marketingAngel.length > 0) {
            productData = {...productData, marketingAngel: marketingAngel}
        }

        if (marketingVideo && marketingVideo.length > 0) {
            productData = {...productData, marketingVideo: marketingVideo}
        }
        ...
        if (targets && targets !== '') {
            productData = {...productData, targets: targets}
        }

        if (advertisementText && advertisementText !== '') {
            productData = {...productData, advertisementText: advertisementText}
        }
